...
跑团存档 schema v4 在顶层保存 `run_id`。alpha.1/v2 旧存档没有身份时，以原始存档文本的 SHA-256 派生稳定的 `legacy_<hash>`，第一次读取后写回顶层和活动遥测，跨重启保持一致。终局提交顺序固定为永久奖励/Profile 落盘、同一局遥测落盘、按 `run_id` 删除本局存档；任一步失败都保留可恢复存档，战败页提供显式重试。
