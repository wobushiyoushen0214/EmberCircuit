...
## 2026-07-16：战斗奖励事务存档与 alpha.8 准备

- 跑团存档 schema 升级到 v5，胜利奖励页可保存本次战斗的精确卡牌、遗物、药水选项、金币显示与三类完成标记。
- 读取奖励页存档时校验 schema、跑团、章节、地图节点、遭遇、金币基线、字段类型和全部奖励 ID；错节点或损坏事务会回滚已入账战利品金币并从迁移存档中清除，不能丢失奖励或通过重打重复拿金币。
- 奖励恢复只从当前数据表按 ID 重建内容；重复读取不会重复增加金币、卡牌、胜场、奖励曝光或路线开始计数，部分领取后再次保存也不会重复发放。
- 载入前会终止旧战斗演出并清空旧战斗引用，迁移写回不再可能把被替换战斗的 HP 写进新存档。
...
## 2026-07-16：通用遗物生产位图扩展

- 使用用户指定的 `gpt-image-2` 接口生成灰烬念珠、沉重齿轮、战鼓残片、熔芯戒指、破盾楔、空白契约、回声石和旧罗盘，统一色键去底并规范化为 `512x512 RGBA PNG`。
- 三名角色的六件开局遗物全部迁移到生产位图；遗物生产位图由 5 件增加到 13 件，继续通过同一资源清单供战斗 HUD、奖励、宝箱、商店和图鉴使用。
- 资源门禁新增遗物槽位与生产数量检查，并验证真实尺寸、alpha 通道和透明像素；临时提示词、色键源图、接口响应和 API 凭据不进入 Git。
- 本轮只替换表现资源，不修改角色、卡牌、怪物、遗物效果、经济、成长或挑战数值。

## 2026-07-16：真人试玩证据门

- 遥测 schema 升级到 v2，cohort 由 schema、游戏版本和配置指纹共同决定；v1 数据迁移为未批准样本，不进入 12/30 数值门。
- 本地留存改为每 cohort/角色/挑战格 40 个完成局和 96 个 abandoned，最多保留最近 4 个 cohort，放弃局不再挤掉完成局。
- 报告按 cohort 独立计算胜率、卡牌 lift、失败集中度和 12/30 覆盖矩阵；顶层兼容字段只映射最新含合格完成局的 cohort，单卡对照另输出两侧各 20 局的样本就绪状态。
