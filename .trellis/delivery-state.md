...
- last_packaged_commit: `f95de1b`（Windows 快照来自 `771cd19`，macOS universal 快照来自 `f95de1b`；不改变数值门结论）
- loop_mode: `L3`
- current_round: `6`
- max_rounds: `6`
...
| REQ-011 | PARTIAL | `export_presets.cfg`, `project.godot`, `packaging/PLAYTEST_README_ZH.txt` | alpha.9-preview Windows PE32+ x86_64 与 macOS arm64/x86_64 universal 均通过启动、包内 SHA、版本、架构、签名/压缩完整性检查；属于用户授权 preview，不代表数值门晋级 | next: native Windows matrix, commercial signing/notarization, installer and Steam integration | L3-post | 0 |
...
- preview_snapshot_macos: `EmberCircuit-0.1.0-alpha.9-preview-macOS-universal-f95de1b.zip`
- preview_snapshot_macos_sha256: `0d752a5d7f2cfb4bac5bd7bf1652db0219d48d45480c0bf1318892be5f95d2a6`
...
- 2026-07-23: 用户追加要求 macOS 试玩包；补充跨平台运行说明后，从 `f95de1b` 导出 arm64/x86_64 universal 应用并完成解压后启动、bundle build 9、临时签名与 SHA 验证。该包同样是 preview 快照，不改变数值门状态。
