...
func _run() -> void:
	...
	_check(int(intro_patrol.get("base_direct_damage_action_count", 0)) == 4 and int(intro_patrol.get("base_action_count", 0)) == 6, "intro patrol has four direct-damage actions across its six-action base cycles")
	_check(is_equal_approx(float(intro_patrol.get("base_attack_action_ratio", 0.0)), 4.0 / 6.0), "intro patrol exposes its base attack-action ratio")
	...
	_check(int(intro_patrol.get("base_first_three_action_damage_total", 0)) == 34, "intro patrol sums both enemies across their first three repeated actions")
	var chapter_one_boss: Dictionary = _row_by_id(monsters, "chapter_one_boss")
	_check(int(chapter_one_boss.get("base_direct_damage_action_count", 0)) == 2 and int(chapter_one_boss.get("base_action_count", 0)) == 5, "chapter one boss has two direct-damage actions in its five-action base cycle")
	_check(is_equal_approx(float(chapter_one_boss.get("base_attack_action_ratio", 0.0)), 0.4), "chapter one boss exposes its low base attack-action ratio")
	_check(int(chapter_one_boss.get("base_longest_zero_direct_damage_actions", 0)) == 2, "chapter one boss exposes its two-action zero-damage window")
	_check(int(chapter_one_boss.get("base_first_three_action_damage_total", 0)) == 15, "chapter one boss first-three pressure begins at action zero")
	_check(int(chapter_one_boss.get("effective_hp_challenge_level", -1)) == 0 and is_equal_approx(float(chapter_one_boss.get("effective_hp", 0.0)), 96.0), "chapter one boss C0 effective HP includes the boss multiplier")
	_check(is_equal_approx(float(chapter_one_boss.get("chapter_highest_elite_effective_hp", 0.0)), 104.0), "chapter one hierarchy records the highest elite C0 effective HP")
	_check(is_equal_approx(float(chapter_one_boss.get("boss_to_highest_elite_ehp_ratio", 0.0)), 0.9231), "chapter one boss-to-elite effective HP ratio exposes the inversion")
	var boss_pressure_issues: Array = chapter_one_boss.get("pressure_issues", [])
	for issue in ["attack_action_ratio_low", "zero_damage_streak_high", "first_three_action_damage_low", "boss_elite_ehp_ratio_low"]:
		_check(boss_pressure_issues.has(issue), "chapter one boss pressure records %s" % issue)
	...
	_check(str(chapter_one_boss.get("pressure_severity", "")) == "warning", "chapter one boss pressure uses an independent severity")
	...
	_check(is_equal_approx(float(ember_exile.get("opening_package_score", 0.0)), 91.38), "ember opening package includes starter deck, deterministic relics, and steel manual")
	_check(is_equal_approx(float(arc_tinker.get("opening_package_score", 0.0)), 82.47), "arc opening package includes base momentum, deterministic relics, and steel manual")
	_check(is_equal_approx(float(pyre_ascetic.get("opening_package_score", 0.0)), 88.41), "pyre opening package includes deterministic opening value without conditional censer damage")
	...
	for player in [ember_exile, arc_tinker, pyre_ascetic]:
		_check(str(player.get("opening_package_severity", "")) == "warning", "opening package over target is an independent warning: %s" % str(player.get("id", "")))
		_check((player.get("opening_package_issues", []) as Array).has("opening_package_high"), "opening package records the high-risk code: %s" % str(player.get("id", "")))
